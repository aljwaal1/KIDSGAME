...
class _SmartMazePageState extends State<SmartMazePage> {
  ...
  Widget build(BuildContext context) {
    final controlsEnabled = !completed && !busy;
    return Scaffold(appBar: AppBar(title: const Text('المتاهة الذكية'), actions: <Widget>[IconButton(onPressed: _reset, icon: const Icon(Icons.refresh_rounded))]), body: ListView(padding: const EdgeInsets.fromLTRB(18, 12, 18, 100), children: <Widget>[
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(borderRadius: BorderRadius.circular(26), gradient: const LinearGradient(colors: <Color>[Color(0xFF22C55E), Color(0xFF14B8A6)])), child: Text(completed ? 'أحسنت! وصلت إلى النجمة 🎉\nالخطوات: $moves' : 'أوصل البطل إلى النجمة\nالخطوات: $moves', style: const TextStyle(color: Colors.white, fontFamily: 'Changa', fontSize: 22, fontWeight: FontWeight.w900))),
      ...
      AspectRatio(aspectRatio: 1, child: GridView.builder(physics: const NeverScrollableScrollPhysics(), itemCount: size * size, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: size, mainAxisSpacing: 6, crossAxisSpacing: 6), itemBuilder: (BuildContext context, int index) {
        ...
        return Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: wall ? const Color(0xFF334155) : isHero ? const Color(0xFFE0F2FE) : isGoal ? const Color(0xFFFEF3C7) : Colors.white, border: Border.all(color: completed && isGoal ? const Color(0xFF22C55E) : const Color(0xFFE2E8F0), width: completed && isGoal ? 3 : 1)), child: Icon(isHero ? Icons.child_care_rounded : isGoal ? Icons.star_rounded : wall ? Icons.block_rounded : Icons.circle_outlined, color: isHero ? const Color(0xFF0EA5E9) : isGoal ? const Color(0xFFF59E0B) : wall ? Colors.white : const Color(0xFFCBD5E1)));
      })),
      const SizedBox(height: 18),
      Column(children: <Widget>[
        IconButton.filledTonal(onPressed: controlsEnabled ? () => _move(-1, 0) : null, icon: const Icon(Icons.keyboard_arrow_up_rounded)),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[
          IconButton.filledTonal(onPressed: controlsEnabled ? () => _move(0, -1) : null, icon: const Icon(Icons.keyboard_arrow_right_rounded)),
          const SizedBox(width: 20),
          IconButton.filledTonal(onPressed: controlsEnabled ? () => _move(0, 1) : null, icon: const Icon(Icons.keyboard_arrow_left_rounded)),
        ]),
        IconButton.filledTonal(onPressed: controlsEnabled ? () => _move(1, 0) : null, icon: const Icon(Icons.keyboard_arrow_down_rounded)),
      ]),
      if (completed) ...<Widget>[
        const SizedBox(height: 12),
        FilledButton.icon(onPressed: _reset, icon: const Icon(Icons.replay_rounded), label: const Text('ابدأ متاهة جديدة')),
      ],
    ]));
  }
}
