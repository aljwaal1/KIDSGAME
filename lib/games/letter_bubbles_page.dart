...
class _BubbleLettersPageState extends State<BubbleLettersPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    final bestText = bestStreak != null && bestStreak! > 0 ? '  •  أفضل متتالية: $bestStreak' : '';
      ...
      child: ListView(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFFE0F7FA), borderRadius: BorderRadius.circular(28), border: Border.all(color: const Color(0xFF67E8F9), width: 2)),
            child: Column(children: [
              const Text('اضغط نفس الحرف', style: TextStyle(color: Color(0xFF0E7490), fontWeight: FontWeight.w800, fontFamily: 'Changa')),
              const SizedBox(height: 8),
              Text(target, style: const TextStyle(color: Color(0xFF155E75), fontSize: 88, fontWeight: FontWeight.w900, height: 1)),
            ]),
          ),
          const SizedBox(height: 10),
          Text('الأخطاء: $mistakes', style: const TextStyle(color: Color(0xFF64748B))),
          ...
          AnimatedBuilder(
            ...
            builder: (context, _) => GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final offset = sin(driftController.value * pi * 2 + index) * 18 * (index.isEven ? 1 : -1);
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          ...
                          gradient: LinearGradient(colors: hidden ? const [Color(0xFFE2E8F0), Color(0xFFCBD5E1)] : const [Color(0xFFA5F3FC), Color(0xFF22D3EE)], begin: Alignment.topRight, end: Alignment.bottomLeft),
                          border: Border.all(color: Colors.white, width: 4),
                          boxShadow: const [BoxShadow(color: Color(0x3306B6D4), blurRadius: 18, offset: Offset(0, 8))],
                        ),
                        child: Center(child: Text(letter, style: const TextStyle(color: Color(0xFF164E63), fontSize: 50, fontWeight: FontWeight.w900))),
                      ),
                    ),
                  ),
                );
