...
class _BattleshipPageState extends State<BattleshipPage> {
  static const n=8; final random=Random(); final confettiKey=GlobalKey<ConfettiOverlayState>(); Timer? timer;
  bool bot=true,placing=true,covered=false; int player=0,placingPlayer=0; int? winner; late List<List<Set<int>>> fleets; late List<Set<int>> shots;
  @override void initState(){super.initState();_reset();}
  List<Set<int>> _fleet(){final ships=<Set<int>>[];for(final length in const [4,3,3,2]){while(true){final horizontal=random.nextBool();final r=random.nextInt(horizontal?n:n-length+1),c=random.nextInt(horizontal?n-length+1:n);final ship=<int>{for(var k=0;k<length;k++)(r+(horizontal?0:k))*n+c+(horizontal?k:0)};if(ships.every((s)=>s.intersection(ship).isEmpty)){ships.add(ship);break;}}}return ships;}
  void _reset(){timer?.cancel();fleets=[_fleet(),_fleet()];shots=[<int>{},<int>{}];setState((){player=0;placingPlayer=0;winner=null;placing=true;covered=false;});}
  void _mode(bool value){if(bot==value)return;bot=value;_reset();}
  bool _shipAt(int owner,int cell)=>fleets[owner].any((ship)=>ship.contains(cell));
  int _remaining(int owner)=>fleets[owner].where((ship)=>!ship.every(shots[1-owner].contains)).length;
  void _confirmFleet(){if(bot){setState((){placing=false;player=0;});}else if(placingPlayer==0){setState((){placingPlayer=1;covered=true;});}else{setState((){placing=false;player=0;covered=true;});}}
  void _fire(int cell,{bool robot=false}){if(placing||winner!=null||covered||(bot&&player==1&&!robot)||shots[player].contains(cell))return;final hit=_shipAt(1-player,cell);setState(()=>shots[player].add(cell));SoundService.instance.play(hit?'wrong.wav':'tap.wav');if(fleets[1-player].every((ship)=>ship.every(shots[player].contains))){setState(()=>winner=player);confettiKey.currentState?.burst(count:36);SoundService.instance.play('win.wav');return;}setState((){player=1-player;covered=!bot;});if(bot&&player==1)_bot();}
